        ...
        try:
            if ext == '.pdf':
                if PYMUPDF_AVAILABLE or PYPDF_AVAILABLE or PDFMINER_AVAILABLE:
                    ...
                    print("  - PDF extraction requires PyMuPDF, pypdf, or pdfminer (none installed)")
                    return ""
            elif ext in ['.docx', '.doc']:
                if DOCX_AVAILABLE:
                    return self._extract_docx(file_path)
                else:
                    print("  - DOCX extraction requires python-docx (not installed)")
                    return ""
            elif ext in ['.xlsx', '.xls', '.csv']:
                if PANDAS_AVAILABLE:
                    return self._extract_excel(file_path)
                else:
                    print("  - Excel extraction requires pandas (not installed)")
                    return ""
            elif ext in ['.pptx', '.ppt']:
                if PPTX_AVAILABLE:
                    return self._extract_pptx(file_path)
                else:
                    print("  - PPTX extraction requires python-pptx (not installed)")
                    return ""
