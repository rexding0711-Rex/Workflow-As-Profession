...
⚠️ 安全警告：本工具使用 exec() 执行用户提供的 Python 脚本以进行性能分析。
被分析的脚本将获得与当前进程完全相同的权限，包括文件系统访问、网络请求和环境变量读取。
请仅在信任的代码上使用本工具，建议在隔离环境（Docker 容器、虚拟机）中运行不受信任的代码。
"""
...
def _prepare_exec(script_path, script_args):
    ...
    ⚠️ 安全注意：此函数使用 exec() 执行用户提供的脚本。
    被分析脚本将获得当前进程的全部权限。
    请仅对受信任的代码使用本工具。
    """
    script_path = Path(script_path).resolve()

    # 基本安全检查：验证文件存在且为 .py 文件
    if not script_path.exists():
        raise FileNotFoundError(f"脚本不存在: {script_path}")
    if script_path.suffix not in ('.py', '.pyw'):
        raise ValueError(f"仅支持 .py/.pyw 文件，收到: {script_path.suffix}")
